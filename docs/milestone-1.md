...
Before erasure, the installer resolves the complete package transaction, revalidates the selected disk, and compares an identity snapshot containing its canonical path, kernel device number, size, model, serial, and WWN with the identity selected by the UI. Physical disks must expose a serial number or WWN; virtual machines may use the complete non-unique token because the open handle below remains authoritative. The UI keeps the selected block device open and passes that descriptor to the engine; metadata erasure and partitioning use the inherited handle, and newly created partitions are opened before formatting and mounting. It rejects live-media, removable, read-only, mounted, stacked, active-swap, and sub-8-GiB disks. Usernames that collide with current accounts, groups, or effective declarations from `/etc`, `/run`, `/usr/local/lib`, or `/usr/lib/sysusers.d` are rejected before disk mutation.
